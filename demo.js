  ...
  this.credentials.authorize((function(computeErr) {
    if (computeErr) {
      ...
      this.credentials = new googleapis.auth.JWT(
      	  "787623176969-78fj0uh875tlb21scifb21864g4cn94b@developer.gserviceaccount.com",
          "/Users/roy/tmp/key.pem",
          SCOPES);
...
Adams.prototype.lookup = function() {
  ...
  this.datastore.lookup({
    ...
    keys: [{ path: [{ kind: 'Trivia', name: 'bla' }] }]
  }).execute((function(err, result) {
    if (err) {
      this.emit('error', {'rpc error': err});
      return;
...
var demo = new Adams("still-toolbox-531");
